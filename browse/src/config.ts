 ...
 *
 * A named instance is a fully-isolated browser: its own state file, its own
 * Chromium profile (separate cookies/logins/sessions), and an auto-picked
 * port. The CLI collapses a `--instance <name>` flag (or GSTACK_INSTANCE env)
 * into BROWSE_STATE_FILE + CHROMIUM_PROFILE, so the daemon core never sees the
 * name — everything flows from the two existing env vars.
 *
 * Instances live globally under <gstackHome>/instances/<name>/ so the same
 * name addresses the same browser from any project directory.
 */

/** Allowed instance-name characters. Disallows '.' and '/', so path traversal is impossible. */
const INSTANCE_NAME_RE = /^[a-zA-Z0-9_-]+$/;

/** Throw a clear error if `name` is empty or contains anything outside [A-Za-z0-9_-]. */
export function validateInstanceName(name: string): void {
  if (!name || !INSTANCE_NAME_RE.test(name)) {
    throw new Error(
      `invalid instance name: ${JSON.stringify(name)} — use only letters, digits, '-' and '_'`,
    );
  }
}

/** Root directory holding all named instances: <gstackHome>/instances. */
export function instancesRoot(env: Record<string, string | undefined> = process.env): string {
  const home = env.GSTACK_HOME || path.join(os.homedir(), '.gstack');
  return path.join(home, 'instances');
}

/**
 * Resolve the state file + profile dir for a named instance.
 * Validates the name first (throws on traversal / bad chars).
 *
 * Note: the profile dir basename is kept as 'chromium-profile' so the
 * cleanSingletonLocks() safety guard recognizes it as a managed profile.
 */
export function resolveInstancePaths(
  name: string,
  env: Record<string, string | undefined> = process.env,
): { stateFile: string; profileDir: string } {
  validateInstanceName(name);
  const dir = path.join(instancesRoot(env), name);
  return {
    stateFile: path.join(dir, 'browse.json'),
    profileDir: path.join(dir, 'chromium-profile'),
  };
}

/** List known instance names (subdirectories of instancesRoot). Empty if root absent. */
export function listInstanceNames(env: Record<string, string | undefined> = process.env): string[] {
  const root = instancesRoot(env);
  try {
    return fs
