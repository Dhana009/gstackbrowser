...
/**
 * Parse a named-instance selector from argv: `--instance <name>` or
 * `--instance=<name>`. Returns the name, or null if absent.
 */
export function parseInstanceName(argv: string[]): string | null {
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--instance') return argv[i + 1] ?? null;
...
/**
 * Collapse a named instance (--instance flag, else GSTACK_INSTANCE env) into
 * BROWSE_STATE_FILE + CHROMIUM_PROFILE so all downstream path resolution flows
 * from the two existing env vars. No-op when BROWSE_STATE_FILE is already set
 * (spawned server child, or explicit override) or when no instance is named.
 * Mutates `env`. Returns the resolved instance name (or null). Throws on an
 * invalid name. Exported for unit tests.
 */
export function applyInstanceEnv(argv: string[], env: NodeJS.ProcessEnv): string | null {
  if (env.BROWSE_STATE_FILE) return null;
  const name = parseInstanceName(argv) ?? env.GSTACK_INSTANCE ?? null;
  if (!name) return null;
  const { stateFile, profileDir } = resolveInstancePaths(name, env);
  env.BROWSE_STATE_FILE = stateFile;
  env.CHROMIUM_PROFILE = profileDir;
  return name;
}

// ─── Named-instance bootstrap (MUST run before resolveConfig) ───
// resolveConfig() reads BROWSE_STATE_FILE; resolveChromiumProfile() reads
// CHROMIUM_PROFILE. Both `config` (below) and the spawned server derive every
// path from those. A named instance is collapsed into them here, at module
// load, before `const config` is evaluated.
let _activeInstance: string | null = null;
try {
  _activeInstance = applyInstanceEnv(process.argv.slice(2), process.env);
} catch (err: any) {
...
// ─── Named Instances ───────────────────────────────────────────
/** Read a state file by absolute path. Returns null if missing/unparseable. */
function readStateAt(stateFile: string): ServerState | null {
  try {
...
/** True if the daemon recorded in `stateFile` has a live PID. */
function isInstanceRunning(stateFile: string): boolean {
  const s = readStateAt(stateFile);
  return !!(s && s.pid && isProcessAlive(s.pid));
}

/** Count named instances whose daemon PID is currently alive. */
function countRunningInstances(): number {
  let n = 0;
  for (const name of listInstanceNames()) {
    try {
      if (isInstanceRunning(resolveInstancePaths(name).stateFile)) n++;
    } catch {
      // ignore unreadable / invalid instance dirs
    }
  }
  return n;
}

/** `browse instances` — list named browsers with port + running/stale status. */
async function listInstancesCommand(): Promise<void> {
  const names = listInstanceNames();
  if (names.length === 0) {
    console.log('No named instances. Create one with: browse --instance <name> <command>');
    return;
  }
  console.log('INSTANCE                 PORT    STATUS');
  for (const name of names.sort()) {
    let port = '-';
    let status = 'stale';
    try {
      const s = readStateAt(resolveInstancePaths(name).stateFile);
      if (s) {
        port = String(s.port ?? '-');
        if (s.pid && isProcessAlive(s.pid)) {
          status = (await isServerHealthy(s.port)) ? 'running' : 'unhealthy';
        }
      }
    } catch {
      status = 'invalid';
    }
    console.log(`${name.padEnd(24)} ${port.padEnd(7)} ${status}`);
  }
}
...
/** Profile dir used by headed/connect Chromium sessions. Honors
 * CHROMIUM_PROFILE (set per named instance) so orphan/lock cleanup targets
 * THIS instance's profile, not the shared default. */
...
export function extractGlobalFlags(rawArgs: string[], env: NodeJS.ProcessEnv): GlobalFlags {
  ...
  for (let i = 0; i < rawArgs.length; i++) {
    ...
    // --instance is consumed by the module-load bootstrap (applyInstanceEnv);
    // strip it here so it never reaches command dispatch / sendCommand.
    if (arg === '--instance') { i++; continue; }
    if (arg.startsWith('--instance=')) { continue; }
    out.push(arg);
  }
...
Refs:           After 'snapshot', use @e1, @e2... as selectors:
  ...
  // ─── Instances (list named browsers) ────────────────────────
  // No server needed — reads each instance's state file and health-checks.
  if (command === 'instances') {
    await listInstancesCommand();
    process.exit(0);
  }

  // Soft warn (no hard cap) when launching a named instance would push the
  // number of live instances past 5. Each Chromium window costs real RAM.
  if (_activeInstance && command !== 'disconnect') {
    const running = countRunningInstances();
    const thisAlreadyUp = isInstanceRunning(config.stateFile);
    if (!thisAlreadyUp && running >= 5) {
      console.error(
        `[browse] warning: ${running} browser instances already running — ` +
        `each Chromium window uses significant RAM`,
      );
    }
  }
  ...
  if (command === 'connect') {
    ...
    try {
      ...
      // Default (unnamed) browser pins the well-known port 34567 so the
      // extension auto-connects with zero handshake. A NAMED instance must
      // NOT pin 34567 (two headed browsers would collide) — it lets the
      // server auto-pick a free port, and the extension adopts that port via
      // the /welcome page handshake in content.js (isolated per profile).
      const serverEnv: Record<string, string> = {
        BROWSE_HEADED: '1',
        ...(_activeInstance ? {} : { BROWSE_PORT: '34567' }),
