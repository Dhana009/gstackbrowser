...
export class BrowserManager {
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      ...
      // Honor CHROMIUM_PROFILE (set per named instance) so a headless→headed
      // handoff keeps the instance's isolated profile instead of falling back
      // to the shared default. Matches launchHeaded() (resolveChromiumProfile).
      const userDataDir = resolveChromiumProfile();
      fs.mkdirSync(userDataDir, { recursive: true });
      cleanSingletonLocks(userDataDir);
