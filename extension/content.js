...
// ─── Daemon port auto-discovery (named instances) ────────────────
// Headed named instances run on their own auto-picked port (not the
// well-known 34567). When this browser opens its daemon's /welcome page,
// adopt the port from the page's own origin so the extension connects to
// THIS browser's daemon instead of the hardcoded default. Each instance has
// its own Chromium profile, so the saved port is isolated per browser.
//
// Gated tightly: only on a loopback host, only on the /welcome path, and only
// after /health confirms a real gstack daemon answers on that port.
(function adoptDaemonPort() {
  try {
    const isLoopback =
      location.hostname === '127.0.0.1' || location.hostname === 'localhost';
    if (!isLoopback || location.pathname !== '/welcome') return;
    const port = parseInt(location.port, 10);
    if (!port) return;

    // Confirm a gstack daemon answers on this port before adopting it.
    fetch(`${location.origin}/health`, { signal: AbortSignal.timeout(2000) })
      ...
      .catch(() => {
        /* not a gstack daemon (or unreachable) — leave the stored port alone */
      });
  } catch {
    /* defensive: never break page load */
  }
})();
